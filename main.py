...
def main():
    ...
    agents.append(Agent("STINKY MCGEE", ComputerScience,
                  buildings, stink=0.5, poi=True))
    ...
    if DEBUG:
        ...
        for building in buildings.values():
            # print(building.name)
            # do not display shadow realm on map as it is not a single point
            if building.name != BuildingName.SHADOW_REALM:
                ax.plot(building.coordinate[1], building.coordinate[0],
                        "ro", markersize=8, label=building.name)
        ...
        ani = animation.FuncAnimation(
            fig, plot_update, frames=1000, interval=100, blit=True)
        ...
        for i in range(tick_count):
            map.update()
            simulation.append({"agents": deepcopy(
                agents), "buildings": deepcopy(buildings), "tick_count": i})
